---
tags:
  - guide/topic
  - status/draft
  - ISG
phase: "[[2 - Transition (2–3 Months Before)]]"
urgency: High
---
# Topic Title

## 1. Overview

- **Timeline:** 3 months before departure
- **Goal:** Research and prepare for procedures that your host institution require you to undertake to start your studies. Deal it with it. (Enrollment, ID cards)
## 2. Importance 

- **The Benefit:** (What do you gain by doing this right?)
- **The Risk:** (What is the penalty/consequence of failing or delaying? e.g., Fines, deportation, homelessness.)

## 3. Prerequisites 
*What you need to have ready before starting.*

- **Documents:**
	- Valid identification document (e.g. Passport)
	- Proof of residence
	- Proof of health Insurance 
	- Proof of Acceptance/Admission to institution
	- Proof of paid fees
	- Proof of language proficiency (e.g. language certificates)
	- Academic Records
	- Letter of Motivation/Purpose/Intent
	- Curriculum Vitae
	- Standardised Test Scores
	- Biometric photos
- **Costs:** 
	- Tuition fees
	- Semester fees
- **Dependencies:** (What specific task must be completed *before* this one?)
	- [[Prepare Core Documents]]
	- [[Source of Funds and Budget]]
	- [[Prepare Proof of Funds]]

## 4. Instructions 
*The step-by-step execution plan.*

1. **Step One:** (Actionable instruction)
2. **Step Two:** (Actionable instruction)
3. **Step Three:** (Actionable instruction)
   
> [!TIP] Pro Tip
> Add a specific "hack" or recommendation here.

## 5. Pitfalls
*Common mistakes and "anti-patterns" to avoid.*

> [!DANGER] Things to Avoid
> - **Mistake 1:** (e.g., Arriving on a Sunday when everything is closed.)
> - **Mistake 2:** (e.g., Using a tourist visa instead of a student visa.)